...
/**
 -----------------------------------------------------------------
 配列を参照で受け取る。
 値には参照型と値型があり、Swiftの配列は値型なので、変数に代入したり、関数の引数で
 渡すと複製された配列が渡される。
 関数の引数に元の配列を指す参照を渡したいときは、関数定義の引数の型の前にinoutを付ける。

 関数を実行するときは、incrimentNums(nums: &引数名)のように引数の前に「&」を付ける。
 -----------------------------------------------------------------
 */
配列を参照で受け取る: do {
    //配列を参照値で受け取る関数incrimentNumsの定義
    ...
    incrimentNums関数に配列dataの参照値を渡す: do {
        var data = [3, 5, 9]
        print(data)             // [3, 5, 9]
        // 実行
